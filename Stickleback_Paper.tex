...
\plr{I like how this starts out, but going to Darwin is maybe a bit too ``big'';
    better to say that we now have a number of examples of rapid evolution;
    cite the Grants and a few others (Gough island house mice? insecticide resistance in Drosophila?),
    then move on to the sticklebacks in more detail.}
...
parallel to freshwater stickleback that have been separated for over $13,000$ years \citep{LescakE7204}.
\plr{clarify what ``parallel'' means}
...
\plr{End with a question that the next two paragraphs propose an answer to.}
...
\plr{Is this ``ecological speciation''? Speciation would imply the ecotypes are developing reproductive incompatibility;
    I think that we think the pattern is stable. Maybe ``adaptation to freshwater habitats''?}
...
% In 2009, Dolph Schluter and Gina L. Conte 
...
Once a new freshwater environment is introduced and inhabited by marine individuals who carry freshwater adapted alleles, selection rebuilds the freshwater haplotype. \cite{schluter2009genetics} 
...
\plr{motivating questions need more work}
%(Other questions)
...
\plr{Should focus on the model, not the software -- describe how the simulation works}
...
\plr{This can be merged with the next section, perhaps as a section called ``Life cycle''.  
    Here and below, avoid using SLiM jargon (don't say ``callback'').}
First, 
...
\plr{no need to say this: we will provide the SLiM recipes as supplementary material
    so that someone interested can see exactly how it works.}
...
\plr{More concisely, these could be called the ``optimal trait values''.}
...
\plr{instead, we can say that we use a ``Gaussian fitness function'', 
...
\plr{Hm. Should we say ``phenotype'' or ``trait''? I've been saying ``trait'', which is less jargon-ey?}
...
\plr{``effect mutation'' is not a standard term; should say ``mutations that affect the phenotype'' here,
    and be careful about what we call them elsewhere.}
