...
Here, we use forward simulations implemented in SLiM that incorporate linkage and realistic length genomes to model the stickleback metapopulation and address these questions \citep{haller2017slim,haller2018slim3}. 
...
We find that only a few stickleback migrants per lake per generation are sufficient to maintain SGV in the ocean, allowing new freshwater populations to adapt in only tens of generations. 
...
\wac{I don't think it's necessary because the census population sizes of stickleback actually due vary widely, so we're not that far off. If anything you could say that we focused on smaller populations because of our questions about how many migrants are needed}
...
\wac{In general I suggest having the finding or statement at the beginning of the sentence with a reference to the figure or statistic at the end of the sentence. Notice that in many cases below (e.g. first sentence of the first paragraph in the 'Local Adaptation' section) the sentence starts with the Figure (e.g. 'As shown in Figure 2'). You can just start with the declarative statement (e.g. "We found that local adaptation occurred in all simulations .... was composed of migrants (Fig. 2)")}
...
\wac{Can we change the header to be a statement of finding? e.g. Local adaptation occurs at almost all levels of gene flow}
...
\wac{same comment as above, is there a declarative statement of finding? e.g. Freshwater adapted alleles are shared extensively and primarily via initial migrants}
...
This is generally because noise of neutral divergence can appear causative when genetic drift causes more 
...
\wac{same thing here - a statement of the major finding of this section?}
...
This suggests that given enough SGV, any reasonable subset of the ancestral population has the potential for rapid adaptation without the need post-colonization hybridization events.
...
\wac{This subsection is a little incongruous with the others sections with respect to how it is written. Seems that the bold headers for the paragraphs was residual from notes? Also, this section seems to be  in a different voice from the rest of the Results/Discussion section. I think it's worth a bit of time trying to hone it more.}
