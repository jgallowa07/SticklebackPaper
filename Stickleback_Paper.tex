...
i.e., about one such mutation every $39375/z$ generations. 
A the begging of the simulation where z = 10, we would then expect the fixation of alleles to take $\approx 5000$ generations.
It follows that when looking at Figure~\ref{fig:phenotype_time}A, we can see the establishment of alleles in jumps roughly this size.
...
has fitness roughly $\exp(-\beta u^2) \approx 1 - \beta u^2$ in the marine environment, i.e a fitness differential of $\beta u^2$. 
The product of population size and fitness differential in the marine environment for a mutation with $u=1$ is therefore $2Ns = 8.9$, 
...
is equal to the total influx of alleles into the ocean per generation divided by the selective disadvantage, i.e.
if there are $I$ immigrants per generation, the frequency is $2 I / \beta u^2$. 
...
With $u=1$, the factor multiplying $I$ is $2/\beta u^2 \approx 1/200$,
so if $I \ge 1$, 
