...
\jgg{one way to do this is check the mean number of samples under the introduced populations?}
...
%Create these once all plots are finalized
Greater migration decreased adaptedness only weakly
except for at the two highest migration rates (10 and 100 migrants per generation per lake),
where the mean trait value equilibrated at roughly half its optimal value and experienced panmixia, respectively.
...
On the left in Figure \ref{fig:MPFAI}B, we see the mean percentage of alleles per individual rapidly increases. 
...
\jgg{todo, make this figure for the supp}
\jgg{this will need updated in I make the plot for it}
Despite a dramatic difference in the amount of allelic sharing between lakes,
standing genetic variance (Figure \ref{fig:SGV}) 
was around 0.05 in the freshwater habitat
