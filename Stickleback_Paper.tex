...
Ten ``effect regions'' of $100,000$ loci each
are spread evenly along the chromosome (separated by $495,0000$ loci)
...
and with effect sizes chosen randomly from an Exponential distribution with mean $1/2$,
...
To fully understand the origins of adaptive variants, calculate statistics, and avoid the computationally expensive task of 
...
\jgg{maybe the below should be in descriptive statistics}
To understand recomposition of the freshwater haplotype in the new lakes through selection, 
we were able to classify each adaptive variant, in each genome in the population at the generation at time of adaptation,
...
\begin{figure}
	\begin{center}
  		...
  		\caption{
		Distribution of freshwater adapted alleles among the three populations. 
...
%$F_{ST}$ between the ``original'' and ``pre-existing'' sets of lakes 
...
In Figure \ref{fig:MPFAI}C we can see that; as migration rates increase, the number of these alleles decreases steadily.
On the left in Figure \ref{fig:MPFAI}B, the mean percentage of alleles per individual rapidly increases. 
Together, these two results strongly suggest the sharing genetic basis.
\jgg{TODO, reference the number of lakes that each allele appears in, make plot for supp}
%As a first indication of this, Figure \ref{fig:MPFAI}B shows that the introduced 
%population utilizes a higher percentage of "pre-existing freshwater adapted alleles"
%with higher rates of introgression between the populations.
Figure \ref{fig:MPFAI}A shows the distribution of the mean number of pre-existing freshwater adapted alleles,
Throughout the simulations, in each deme.
...
\begin{figure}[h!tb]
	\begin{center}
  		\includegraphics[width=0.6\linewidth]{semi_final_plots/PhenoDist_semi_final.png}
...
\begin{figure}
	\begin{center}
  		\includegraphics[width=0.7\linewidth]{semi_final_plots/OriginRatio.png}
  		\caption{ 
...
%Now lets make this more concrete
