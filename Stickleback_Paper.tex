...
%The simulation begins with two subpopulations, one of both the
Throughout the simulation there are two habitat types defined by their selective pressures.
...
%The two habitat types -- made to model the marine and freshwater selective pressures --  a fixed total populations size of 5,000 diploid individuals each. 
%The habitat types differ in the optimal trait value.
%The simulations begin with two subpopulations, each of size 5,000 diploid individuals, one set living in marine and freshwater selective pressures.
...
In short, the difference between an individual's trait value and the optimum is the main determinant of an individuals fitness.
...
\textbf{This next paragraph needs work, skipping for now}
Dispersal of individuals occurs both locally along the coastline in the marine habitat 
and between the marine habitat and the lakes, and can be thought of as occurring at the juvenile stage (before the selection of parents). 
There is no dispersal directly between, or within, lakes.
The bi-directional lake--ocean migration rate is denoted $m$, 
which we refer to as the rate of \emph{gene flow} between habitats.
Parents for a new lake individual are chosen from the marine habitat with probability $m$:
to obtain the pair of freshwater parents, a first parent is chosen proportional to fitness, and a mate is chosen from the same lake as the first, also proportional to fitness. 
