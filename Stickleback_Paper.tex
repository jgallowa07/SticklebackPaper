...
Interestingly, in Figure \ref{fig:MPFAI}A at $0.1$ \emph{mig/gen/lake}, we see sharing of alleles between populations,
...
perhaps a larger influx of adapted alleles are needed in addition to standing variation in the initial generation.
...
Surprisingly in Figure \ref{fig:Origin}, we find that at both $m = 0.1$ and $m = 1.0$ \emph{migs/gen/lake},
the vast majority of adaptive alleles origins were found to propagate from individuals in the original generation ("Captured" OR "Pre-Existing").
...
In turn, the equilibrium of freshwater alleles in marine genomes created by migration-selection balance at lower rates of gene flow acts as a barrier when rebuilding the freshwater haplotype. 
With more gene flow between the two selective pressures, the freshwater haplotype would remain more "intact" and much more efficient to rebuild in the introduced environment.
...
Given the probability that effect region fixes at a given trait value, $2 \times x / 45$ where $x$ is the effect size of a haplotype
